...
|---|---|
| `docs/open-issues.md` | **Read before starting anything.** 34 issues — 25 closed (one partly), 9 open. The closed ones record decisions you would otherwise re-litigate, and several record a bug's *shape* rather than just its fix. |
...
`examples/modules/` holds ten working modules. **They are tested** (`daemon/tests/unit/test_examples.py`) and are the closest thing to a public API conformance suite: a change that breaks a module written the documented way breaks there.
...
Open issues live in `docs/open-issues.md`. Currently open: OI-6, OI-10 (remaining half), OI-12, OI-13, OI-15, OI-20, OI-21, OI-27, OI-36.
