...
about. It reads oddly if you expect a receiver first, and it is what the engine
calls: `fn(request, context)`, `fn(request, response, context)`,
`fn(message, request, context)`. Getting it backwards raises on the first flow,
and the error blames the module's author. Every module in `examples/modules/`
is written this way and is the thing to copy.

`on_report(ctx)` is not a flow hook — it is called on demand from the control
